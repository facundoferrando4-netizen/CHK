...
OWNER_ID = 8016112409  # Owner's ID
...
async def ensure_owner_is_authorized():
    if not await authorized_users_collection.find_one({"id": 8016112409}):
...
async def get_user_rank(user_id):
    ...
    if not user:
        return "Free user"
    if user.get("role") == "owner" or user == 8016112409:
        return "Dev"
...
async def assign_roles_to_all():
    ...
    for user in all_users:
        role = "owner" if user["id"] == 8016112409 else ("authorized" if await is_user_authorized(user["id"]) else "free")
