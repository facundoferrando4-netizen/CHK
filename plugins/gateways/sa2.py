...
from Gates.sa2 import stripe_auth2
...
import re,time
@Client.on_message(filters.command("sa2" , prefixes=["/", "."]))
async def sa(client, message):
    user_id = message.from_user.id
    first_n = message.from_user.first_name
    mid = message.id

    if get_command_status('sa2') == 'off':
    ...
    # Check if command is used in a group and if the group is authorized
    #roup is authorized
...
#            await message.reply_text("This group is not authorized to use this command.", reply_to_message_id=mi
...
'''
    ...
    ccc = None
    if len(message.text.split('sa2 ')) > 1 and message.text.split('sa2 ')[1].strip():
            ccc = message.text.split('sa2 ')[1].strip()
...
''', quote=True, disable_web_page_preview=True)
    ...
    msg = await stripe_auth2(cc,mm,yy,cvv)
